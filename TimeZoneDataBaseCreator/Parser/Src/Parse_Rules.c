...
STATIC VOID Free_Rule_Year_Data(Rule_Year_Data_t** standards, COUNTER* count);
...
STATIC VOID Free_Rule_Year_Data(Rule_Year_Data_t** year_data, COUNTER* count)
{
    if (year_data != NULL && *year_data != NULL && count != NULL)
    {
        for (COUNTER i = 0; i < *count; i++)
        {
            if ((*year_data)[i].Letter != NULL)
            {
                free((*year_data)[i].Letter);
            }
            if ((*year_data)[i].Comment != NULL)
            {
                free((*year_data)[i].Comment);
            }
        }
        free(*year_data);
        *year_data = NULL;
    ...
    if (rule != NULL)
    {
        ...
        if (rule->Years != NULL && rule->Years_Count > 0)
        {
            for (COUNTER year_index = 0; year_index < rule->Years_Count; year_index++)
            {
                ...
                {
                    Free_Rule_Year_Data(&year->Standard, &year->Standard_Count);
                }
                if (year->DST != NULL && year->DST_Count > 0)
                {
                    Free_Rule_Year_Data(&year->DST, &year->DST_Count);
                }
            }
            free(rule->Years);
        }
